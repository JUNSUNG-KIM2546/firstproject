...
class CommentRepositoryTest {
    ...
    void findByArticleId() {
        ...
        // Case 3: 9번 게시글의 모든 댓글 조회 [셀프체크]
        {
            ...
            // 3. 예상 데이터
            Article article = new Article(9L, "당신의 인생 영화는?", "댓글 고");
            Comment a = new Comment(10L, article, "Park","오직 그대만");
            Comment b = new Comment(11L, article, "Kim","너의 이름은");
            Comment c = new Comment(12L, article, "Choi","뷰티 인사이드");
            List<Comment> expected = Arrays.asList(a, b, c);
        ...
        // Case 4: 999번 게시글의 모든 댓글 조회 [셀프체크]
        {
            // 1. 입력 데이터 준비
            Long articleId = 999L;
            ...
            // 3. 예상 데이터
            Article article = new Article(999L, "토요일 점심은 쌀국수", "1");
            List<Comment> expected = Arrays.asList();
    ...
    void findByNickname() {
        ...
        // Case 2: "Kim"의 모든 댓글 조회
        {
            ...
            // 3. 예상 데이터
            Comment a = new Comment(2L, new Article(4L,"당신의 인생 영화는?", "댓글 고"), nickname, "너의 이름은"); // 댓글 객체 생성 (부모 객체는 각 필드에 따로 생성)
            Comment b = new Comment(5L, new Article(5L,"당신의 소울 푸드는?", "댓글 고고"), nickname, "생선구이");
            Comment c = new Comment(8L, new Article(6L,"당신의 취미는?", "댓글 고고고"), nickname, "노래방");
            List<Comment> expected = Arrays.asList(a, b, c);    // 댓글 객체 합치기

            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "Kim의 모든 댓들을 출력!");
        }
        ...
        // Case 3: null의 모든 댓글 조회
        {
            ...
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "null의 모든 댓들을 출력!");
        }
        ...
        // Case 3: ""의 모든 댓글 조회
        {
            ...
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "\"\"의 모든 댓들을 출력!");
        }
    }
}